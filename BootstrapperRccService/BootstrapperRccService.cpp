...
void BootstrapperRccService::installService()
{
	...
	const TCHAR rbxRegPath[] = _T("Software\\ProjectX Corporation\\ProjectX");
	CRegKey pathKey;
	pathKey.Create(HKEY_LOCAL_MACHINE, rbxRegPath);
	if (pathKey.m_hKey == NULL)
		throw std::runtime_error("Unable to create/open reg key: 'Software\\ProjectX Corporation\\ProjectX'");

	LONG res = pathKey->SetStringValue(_T("RccServicePath"), (LPCTSTR)ProgramDirectory().c_str());
	if (res != ERROR_SUCCESS)
		throw std::runtime_error("Failed writing to registry: 'Software\\ProjectX Corporation\\ProjectX\\RccServicePath'");
}
